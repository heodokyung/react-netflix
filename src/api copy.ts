...
export const getSearchResult = async ({
	...
	// return await axios(
	// 	`https://api.themoviedb.org/3/search/${category}?&api_key=${API_KEY}&language=ko-KR&query=${keyword}&page=${1}`,
	// ).then((response) => response.data);
};
*/
...
export async function getSearchResult({
	...
	return (
		await fetch(
			`${BASE_URL}/search/${category}?api_key=${API_KEY}&language=en-US&query=${keyword}&page=${page}`,
		)
	).json();
}
